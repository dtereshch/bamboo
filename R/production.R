...
#' entry, exit, ownership changes, and a completely missing wave (year 4).
#' The data follows industry-specific production structures with occasional
#' industry and ownership changes.
#'
#' @format A data frame with 210 rows (30 firms × 7 years) and 7 variables:
#' \describe{
#'   \item{firm}{integer; firm identifier (1 to 30)}
#'   \item{year}{integer; year identifier (1,2,3,4,5,6,7). Year 4 contains only
#'               missing values for all firms.}
#'   \item{industry}{factor; industry affiliation with three levels:
#'                   "Industry 1", "Industry 2", "Industry 3". Some firms change
#'                   industry over time. Set to NA for inactive periods and year 4.}
...
#'                technology shocks. Contains random missing values (~2\%) and
#'                is NA for inactive periods and year 4.}
#'   \item{capital}{numeric; capital input, log‑normally distributed with
#'                  firm-specific effects and industry-specific time trends.
#'                  Contains random missing values (~2\%) and is NA for inactive
#'                  periods and year 4.}
...
#'                    Set to NA for inactive periods and year 4.}
#' }
#'
#' @details
#' The dataset exhibits several realistic features of firm-level panel data:
#' \itemize{
#'   \item 50\% of firms (15 firms) have complete data for all years (except year 4).
...
#'   \item Year 4 is completely missing (all variables NA) for all firms.
...
#' data(production)
#' head(production)
#' table(production$year)  # year 4 has only NAs
"production"
